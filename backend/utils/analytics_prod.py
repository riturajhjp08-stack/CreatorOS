...
def refresh_instagram_token(platform):
    ...
    try:
        ...
        platform.token_expires_at = datetime.utcnow() + timedelta(seconds=data.get('expires_in', 5184000))
        platform.updated_at = datetime.utcnow()
        db.session.commit()
        return True
    except:
...
def sync_youtube_analytics(user_id, platform_id, access_token):
    ...
    try:
        ...
        channels_params = {'part': 'statistics,snippet', 'mine': 'true'}
        ...
        videos_params = {'part': 'statistics', 'forMine': 'true', 'maxResults': 50}
...
def sync_tiktok_analytics(user_id, platform_id, access_token):
    ...
    try:
        ...
        stat_params = {'fields': 'follower_count,video_count,heart_count,download_count,share_count,comment_count'}
        ...
        try:
            ...
            videos_params = {'fields': 'id,create_time,view_count,like_count,comment_count,share_count'}
            videos_response = requests.get(videos_url, headers=headers, params=videos_params)
            videos_data = videos_response.json()
...
def sync_twitter_analytics(user_id, platform_id, access_token):
    ...
    try:
        ...
        user_params = {'user.fields': 'public_metrics,created_at,followers_count'}
